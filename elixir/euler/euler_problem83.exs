defmodule PathSum do
  ...
  def solution() do
    {:ok, content} = File.read("p083_matrix.txt")
    ...
    get_min_path(list, matrix, {0, 0}) |> Enum.filter(fn {x, y, _, _} -> x == 79 and y == 79 end)
  end
  ...
  def get_distance([], _x, _y), do: 1000000000
  ...
  defp bpm_out(matrix, l, x, y, state) do
    ...
    bpm_out(matrix, l, x+1, y, state |> Map.put({x, y},
	    [{x + 1, y, right, :uncertain}, {x, y + 1, down, :uncertain}, {x, y - 1, up, :uncertain}, {x-1, y, left, :uncertain}]))
  end
