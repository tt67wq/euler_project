defmodule PokerHands do
  ...
  def counter(nums), do: nums |> Enum.reduce(%{}, fn x, acc -> Map.update(acc, x, 1, fn x -> x + 1 end) end)
  ...
  def get_level(cards) do
    values = cards |> Enum.map(fn x -> String.at(x, 0) end) |> Enum.map(fn x -> Map.fetch!(@value_map, x) end)
  ...
  def compare(cards1, cards2) do
    ...
    values1 = cards1 |> Enum.map(fn x -> String.at(x, 0) end) |> Enum.map(fn x -> Map.fetch!(@value_map, x) end)
    values2 = cards2 |> Enum.map(fn x -> String.at(x, 0) end) |> Enum.map(fn x -> Map.fetch!(@value_map, x) end)
    cond do
      level1 < level2 -> 1
      level1 > level2 -> 0
      :else ->
  ...
  def get_card_data_from_file(path) do
    ...
    String.split(content, "\n") |> Enum.map(fn x -> String.split(x) end) |> Enum.map(fn x -> ten_to_five(x) end) |> Enum.filter(fn x -> x != nil end)
  end

  def solution() do
    get_card_data_from_file("poker.txt") |> Enum.map(fn {a, b} -> compare(a, b) end) |> Enum.filter(fn x -> x == 1 end) |> length()
  end
end
