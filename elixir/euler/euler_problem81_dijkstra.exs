defmodule PathSum do
  @moduledoc """
  resolve euler 81 in dijkstra way
  """
  require Logger
  ...
  def solution() do
    ...
    v = get_matrix_elem(list, 0, 0)
    acc = Map.fetch!(matrix, {0, 0}) |> Enum.map(fn {x, y, v1, :uncertain} -> {x, y, v1 + v, :uncertain} end)
    iter(matrix, acc) |> get_acc_elem({79, 79})
  end
  ...
  defp bpm_out(matrix, l, x, y, state) do
    ...
    bpm_out(matrix, l, x+1, y, state |> Map.put({x, y}, [{x + 1, y, right, :uncertain}, {x, y + 1, down, :uncertain}]))
  end


  # defp bpm_in(_matrix, l, l, y, state) when y > l, do: state
  # defp bpm_in(matrix, l, x, y, state) when x > l, do: bpm_in(matrix, l, 0, y + 1, state)
  # defp bpm_in(matrix, l, x, y, state) do
  #   left = matrix |> get_matrix_elem(x - 1, y)
  #   up  = matrix |> get_matrix_elem(x, y - 1)
  #   bpm_in(matrix, l, x+1, y, state |> Map.put({x, y}, [{x - 1, y, left, :uncertain}, {x, y - 1, up, :uncertain}]))
  # end
